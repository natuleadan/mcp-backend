...
export const bootstrapTools = [
  {
    ...
    description: "Bootstrap the Iceberg FDW on Postgres: creates namespace + tables in catalog, configures FDW server, maps foreign tables, and verifies connection.",
    ...
    handler: async ({ dry_run }: { dry_run: boolean }) => {
      ...
      try {
        ...
        // Create tables
        const ICEBERG_TABLES: Record<string, { type: "struct"; fields: { id: number; name: string; type: string; required: boolean }[]; "schema-id": number }> = {
          aud_registry: { type: "struct", "schema-id": 0, fields: [
          ...
          aud_analytics: { type: "struct", "schema-id": 0, fields: [
            { id: 1, name: "id", type: "uuid", required: false },
          ...
          aud_storage: { type: "struct", "schema-id": 0, fields: [
            { id: 1, name: "id", type: "uuid", required: false },
          ...
          aud_ip_blocks: { type: "struct", "schema-id": 0, fields: [
            { id: 1, name: "id", type: "uuid", required: false },
          ...
          aud_cookies: { type: "struct", "schema-id": 0, fields: [
            { id: 1, name: "id", type: "uuid", required: false },
            { id: 2, name: "user_id", type: "uuid", required: false },
            { id: 3, name: "anonymous_id", type: "uuid", required: false },
            { id: 4, name: "ip_hash", type: "string", required: false },
        ...
        for (const [tableName, schema] of Object.entries(ICEBERG_TABLES)) {
          try {
            await catalog.createTable(
              { namespace: [NAMESPACE] },
              { name: tableName, schema, "partition-spec": { "spec-id": 0, fields: [] }, "write-order": { "order-id": 0, fields: [] }, properties: { "write.format.default": "parquet" } }
            );
