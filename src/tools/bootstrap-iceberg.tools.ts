...
export function registerBootstrapIcebergTool(server: McpServer) {
  server.tool(
    'bootstrap_iceberg',
    ...
    async ({ dry_run }: { dry_run: boolean }) => {
      try {
        ...
        if (!config.catalogUri || !config.icebergToken) {
          return {
            content: [{ type: 'text', text: '❌ Missing CATALOG_URI or ICEBERG_TOKEN in .env' }],
            isError: true,
          }
        }
  ...
  id uuid, user_id uuid, anonymous_id uuid, ip_hash text, country_code text, lang_param text, consent_categories text, user_agent text, created_at timestamptz
      ...
      `
        ...
        if (dry_run) {
          ...
          log.push('CREATE EXTENSION IF NOT EXISTS wrappers WITH SCHEMA extensions;')
          log.push('DROP SERVER IF EXISTS iceberg_server CASCADE;')
          log.push('CREATE SERVER iceberg_server')
          log.push('  FOREIGN DATA WRAPPER iceberg_wrapper')
          log.push('  OPTIONS (')
          log.push("    aws_access_key_id '[REDACTED - from env]',")
          log.push("    aws_secret_access_key '[REDACTED - from env]',")
          log.push("    token '[REDACTED - from env]',")
          log.push("    region_name 'us-east-1',")
          log.push("    catalog_uri '" + config.catalogUri + "',")
          log.push("    warehouse '" + WAREHOUSE + "',")
          log.push('    "s3.endpoint" \'' + config.s3Endpoint + "'")
          log.push('  );')
          log.push('[... foreign tables creation omitted ...]')
          return {
        ...
        try {
          const r1 = await testClient.query(
            'SELECT COUNT(*) as cnt FROM iceberg.aud_analytics_lake'
          )
